...
\begin{itemize}
    ...
    \item We show experimentally that the non-IID axis, not merely the presence of non-IID data, decides whether federation outperforms isolated training, and we report the negative result for the attack-campaign split alongside the nuanced positive one for the traffic-domain split (Section~\ref{sec:design-space}).
...
Mean $\pm$ standard deviation over eight seeds. Edge~A serves web traffic,
Edge~B serves API traffic.}
\label{tab:detection}

\resizebox{\columnwidth}{!}{%
\begin{tabular}{@{}lrrrrrr@{}}
\toprule
\textbf{Model} & \textbf{ROC-AUC} & \textbf{PR-AUC} & \textbf{XSS} & \textbf{SQLi} & \multicolumn{2}{c}{\textbf{FPR}} \\
 & & & \multicolumn{2}{c}{\footnotesize @0.1\% FPR} & \footnotesize web & \footnotesize api \\ \midrule
Edge A & 0.9613$\pm$0.0070 & 0.9664$\pm$0.0059 & 0.845$\pm$0.085 & 0.692$\pm$0.079 & 0.0008 & 0.0045 \\
Edge B & 0.9681$\pm$0.0055 & 0.9694$\pm$0.0051 & 0.780$\pm$0.106 & 0.642$\pm$0.087 & 0.0092 & 0.0017 \\
Federated & \textbf{0.9688$\pm$0.0061} & \textbf{0.9719$\pm$0.0052} & \textbf{0.888$\pm$0.050} & \textbf{0.744$\pm$0.049} & \textbf{0.0022} & \textbf{0.0003} \\ \bottomrule
\end{tabular}%
}
\end{table}
...
\caption{Per-class detection on payloads absent from every node's training set. The federated model delivers the most balanced profile across both attack classes and both benign domains.}
\label{fig:metrics}
\end{figure}

Table~\ref{tab:detection} and Fig.~\ref{fig:metrics} report the headline configuration. To account for run-to-run variance, we report mean $\pm$ standard deviation across eight initialisation seeds. The federated model achieves the highest average ROC-AUC (0.9934 $\pm$ 0.0021) and XSS recall (0.966 $\pm$ 0.029), matching the strongest silo (Edge~B) within the margin of error. On SQLi recall, the federated model averages 0.839 against Edge~B's 0.831, slightly outperforming the best isolated silo.

Federation's primary value is providing privacy at no measurable accuracy cost, while ensuring cross-domain generalisation. Edge~A alone produces a prohibitive 0.62\% false-positive rate on API traffic and reaches only 0.806 SQLi recall. After federation, its effective model lifts to 0.865 SQLi recall and zeros its cross-domain FPR, achieving the most balanced profile across both attack classes and both benign domains.

The design-space matrix (Section~\ref{sec:design-space}) further makes explicit that this generalisation is contingent on how heterogeneity is distributed.

\begin{figure}[htbp]
\centering
\includegraphics[width=\columnwidth]{fl_sweep.pdf}
\caption{Left: convergence of the global model. Right: accuracy against synchronisation payload size as the $n$-gram dimension $D$ varies. The federated model balances detection across domains at every $D$.}
\label{fig:sweep}
\end{figure}

Fig.~\ref{fig:sweep} (left) shows the global model reaching within 0.001 of its best ROC-AUC by round~3, so the twenty rounds we budget are ample; rapid convergence matters for ZTA because it bounds how long a newly observed threat takes to propagate. Fig.~\ref{fig:sweep} (right) sweeps $D$ from 32 to 512. The federated model consistently delivers the most balanced cross-domain profile at every dimension tested, and the curve gives the accuracy-versus-bandwidth trade-off directly: $D=256$ costs 1{,}040 bytes per update, $D=512$ buys additional SQLi recall for 2{,}064.
...
The benefit of federation is not an absolute ROC-AUC improvement over every isolated silo, but rather privacy at no measurable accuracy cost, combined with cross-domain generalisation. Across eight initialisation seeds, federation matches the strongest silo within the margin of run-to-run variance, while lifting Edge~A's SQLi recall from 0.801 to 0.839 and eliminating its prohibitive 0.62\% cross-domain false-positive rate. This gain is contingent on the heterogeneity residing in the benign traffic distribution; practitioners partitioning by threat campaign should expect federated averaging to land between their silos rather than above them.

SQLi recall of 0.839 at a 0.1\% false-positive budget remains the weakest result, though it represents a substantial improvement over the 0.792 achieved with the original 260-payload corpus. Expanding to 681 unique SQLi payloads from 17 SecLists dictionaries improved both isolated and federated models, but narrowed the gap that federation was closing. With 205 test payloads (up from 78), the confidence interval is materially tighter.
...
Beyond these, compiling the inferencer to WebAssembly would provide sandbox isolation against model-extraction attacks while retaining sub-millisecond latency. The confidence-thresholded pseudo-labelling originally used by the continuous-learning loop has been replaced by a frozen anchor-set replay mechanism, which prevents model drift but limits adaptation to the cadence of federated synchronisation; integrating a principled active-learning criterion with human-in-the-loop verification would restore online adaptability without reintroducing the drift risk.

\section*{Data and Code Availability}
The curated dataset manifests, the federated simulation, the Byzantine poisoning experiment, the pure-Python edge inferencer and the testbed instructions are available at \url{https://github.com/eldemaster/FedZTA-Edge}.
...
\bibitem{friend_pqtls} A. De Martini, ``Benchmarking Classical and Post-Quantum TLS on Raspberry Pi,'' \textit{Mid Sweden University}, 2023.
\end{thebibliography}
